...
#else // #if (DDR_WRITE_LEVELING_EN == 1)
int ddr_hw_write_leveling(void)
{
	MEMMSG("Do not support H/W Write Leveling!! \r\n");
	return 0;
}
...
void write_latency_information(void)
{
	...
	status = ReadIO32(&pReg_DDRPHY->CAL_WL_STAT) & 0xF;
	if (status != 0) {
		latency = (ReadIO32(&pReg_DDRPHY->PHY_CON[4]) >> 16) & 0x1F;
		latency_plus = (ReadIO32(&pReg_DDRPHY->PHY_CON[5]) >> 0) & 0x7;
...
int ddr_write_latency_calibration(void)
{
...
#if 0
	/* Step XX.  unknown */
	mmio_clear_32(&pReg_DDRPHY->PHY_CON[0], (0x1 << 14));			// p0_cmd_en[14] = 0
	mmio_set_32  (&pReg_DDRPHY->PHY_CON[0], (0x1 << 14));			// p0_cmd_en[14] = 1
#endif
	/* Step 03. Set the colum address*/
	...
	for (cal_count = 0; cal_count < 100; cal_count++) {
		response = ReadIO32( &pReg_DDRPHY->PHY_CON[3] );
	...
	done_status = ReadIO32(&pReg_DDRPHY->CAL_WL_STAT) & 0xF;
	if (done_status != 0xF) {
...
int ddr_write_latency_calibration(void)
{
	MEMMSG("Do not support Write Latency Calibration!! \r\n");
	return 0;
}
#endif // #if (DDR_WRITE_LEVELING_EN == 1)
